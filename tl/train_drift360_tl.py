...
MODEL_DIR = "models/drift360"
LOG_DIR = "logs/drift360"
...
class SingleVecToGymWrapper(gym.Env):
    ...
    def step(self, action):
        ...
        if isinstance(info, dict):
            info = {k: v[0] if isinstance(v, (list, tuple)) and len(v) > 0 else v for k, v in info.items()}
        else:
            info = {}
...
def make_env():
    vec_env = gym.make_vec(
        ...
        initializer="zero", # zmienic na staly
        compile=False,
    )
...
def main():
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=25_000,
        save_path=MODEL_DIR,
        name_prefix="ppo_drift360_ent_reg_0_01",
    )
    ...
    model = PPO(
        ...
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.01,  # <--- TUTAJ DODAJESZ ENTROPIĘ (np. 0.01)
        verbose=1,
    ...
    model.learn(
        ...
        tb_log_name="ppo_drift360_ent_reg_0_01",
    )

    model.save(os.path.join(MODEL_DIR, "ppo_drift360_ent_reg_0_01_final"))
    env.close()
