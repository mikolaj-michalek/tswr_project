...
class F1tenthDataset(torch.utils.data.Dataset):
    def __init__(self,
                 dataset_path: str,
        ...
        U_cols = ["omega_wheels", "omega_wheels", "delta"]
        
        # State vector columns
        #X_cols = ["v_x", "v_y", "r", "friction"]
        #X_cols = ["v_x", "v_y", "r"]
        X_cols = ["v_x", "v_y", "r", "friction", "friction"]
        #X_cols = ["x", "y", "yaw", "v_x", "v_y", "r"]

        # Observation columns
        #M_cols = ["v_x", "v_y", "r", 
        #          "omega_wheels", "delta"]
        M_cols = X_cols + U_cols
        
        # Position columns (for evaluation / visualisation only, not used in training)
        P_cols = ["x", "y", "yaw"]
        ...
        # steering angle is between -0.5 and 0.5
        self.U[:, -1] = torch.clip(self.U[:, -1], -0.5, 0.5)
        self.M[:, -1] = torch.clip(self.M[:, -1], -0.5, 0.5)
...
#        plt.show()
    ...
    def _check_moving(self, t_minus_to, t_plus_tp, min_vx_speed = 1., min_wheel_speed = 1.):
        ...
        wx = StateWrapper(x)
        moving = torch.all((wx.v_x) > min_vx_speed) and torch.all((wx.omega_wheels_front) > min_wheel_speed)
        return moving
    ...
    def __getitem__(self, idx):
        ...
        U = torch.stack([self.U[t + self.shift_u1:t_plus_tp + self.shift_u1, 0],
                         self.U[t + self.shift_u1:t_plus_tp + self.shift_u1, 1],
                         self.U[t + self.shift_u2:t_plus_tp + self.shift_u2, 2]], dim=-1)
