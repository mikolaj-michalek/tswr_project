...
from ldm.systems.car.dynamics.kicajka5_params import Kicajka5Parameters
...
class Kicajka5TireModel(BaseTireModel):
    def __init__(self, n, n_up, *args, **kwargs) -> None:
        super().__init__()
        self.front_tire_model_parameters = Kicajka5Parameters(n, n_up)
        self.rear_tire_model_parameters = Kicajka5Parameters(n, n_up)
    ...
    def tire_forces_model(self, slip_angle_rad, slip_ratio, wp):
        ...
        Sx_final = Sx_mod + wp.sr_offset / 10.
        Fx = compute_force(self.bspline, Sx_final, wp.x_scale, Sx_norm, S_resultant, wp.cps_x) + wp.Fx_offset / 10.
        Alpha_final = Alpha_mod + wp.sa_offset / 10.
        Fy = compute_force(self.bspline, Alpha_final, wp.y_scale, Alpha_norm, S_resultant, wp.cps_y) + wp.Fy_offset / 10.
        return Fx, - Fy
    ...
    def tire_forces(self, slip_ratio_front, slip_angle_front,
        ...
        # t = clip((S * norm + offset/10) * scale, 0, 1)  — same as tire_forces_model
        t_xf = torch.clip((S_f * wp_f.x_norm + wp_f.sr_offset / 10.) * wp_f.x_scale, 0., 1.)
        t_yf = torch.clip((S_f * wp_f.y_norm + wp_f.sa_offset / 10.) * wp_f.y_scale, 0., 1.)
        t_xr = torch.clip((S_r * wp_r.x_norm + wp_r.sr_offset / 10.) * wp_r.x_scale, 0., 1.)
        t_yr = torch.clip((S_r * wp_r.y_norm + wp_r.sa_offset / 10.) * wp_r.y_scale, 0., 1.)
        ...
        # Add output offsets; fric_yf/fric_yr are already negated by compute_forces_quad
        fric_xf = fric_xf + wp_f.Fx_offset / 10.
        fric_yf = fric_yf - wp_f.Fy_offset / 10.
        fric_xr = fric_xr + wp_r.Fx_offset / 10.
        fric_yr = fric_yr - wp_r.Fy_offset / 10.
