...
from ldm.systems.car.casadi_dynamics.neural_exp_tanh_tire_model import CasadiNeuralExpTanhTireModel
...
class CasadiSingleTrackCoupled:
    ...
    def load_parameters_from_model(self, model_path):
        ...
        self.tire_model.load_from_torch_model(dynamics_model.tire_model)
        self.friction = dynamics_model.log_friction.exp().item()
        self.build_casadi_functions()

    def forward(self, x, u, vehicle_params, friction_coeff):
        """
        x: [v_x, v_y, r]
        ...
        raw_tire_forces = self.tire_model.forward(x_and_u, wp)
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(os.path.dirname(__file__), "../../../../experiments/paper/f1tenth_new_fixed/prediction_models",
                              #"neural_exptanh_single_track_None_gc0.01_lr0.0005_hf100_100_hb100_str2_euler_nn4_1.0_nodrag_nolr_seed0.pt")
                              "neural_single_track_None_gc0.01_lr0.0005_hf100_100_hb100_str2_euler_nn16_1.0_nodrag_nolr_seed0.pt")
    dynamics_model, _ = load_dynamics_model(model_path)

    #tire_model = CasadiNeuralExpTanhTireModel()
    tire_model = CasadiNeuralTireModel()
    dynamics = CasadiSingleTrackCoupled(tire_model)
    dynamics.load_parameters_from_model(model_path)

    x = np.array([4.0, 0.0, 0.1])[None]  # Example state
    u = np.array([5.0, 5.0, 0.0])[None]  # Example control
